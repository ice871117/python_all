...
if len(sys.argv) > 1:
    ...
    for i in range(0, nrows):
        ...
        try:
            brand = lastCol[lastCol.index("and_fm_") + 7:]
            brand = brand[0 : 5]
    ...
    for key, value in brandMap.items():
        ...
        index += 1
    destFile = None
    if fileName.startswith("/") or fileName.find("/") >= 0:
        destFile = '/Users/wiizhang/downloads/version_result_' + fileName[fileName.rindex("/") + 1:]
    else:
        destFile = '/Users/wiizhang/downloads/version_result_' + fileName
    if destFile.find(".") >= 0:
